...
- 模块：`seuLex`
- 测试执行时间：`2026-04-14`
- 本次结果目录：`tests/lex/results/20260414-183656`
- 执行脚本：`tests/lex/run_lex_tests.sh`
- 总体结果：`13 / 13` 通过

本测试集仅新增测试资产和测试文档，不修改 `seuLex` 现有实现。测试目标是把当前代码已经实现的能力做成可复跑、可归档、可审阅的黑盒/灰盒验证集合。
...
- 生成 lexer 的编译与运行
- 负向错误处理
- `resources/minic.l` / `resources/c99.l` 生成回归
- CLI `--self-test`
- 长输入运行性能
...
| 回归 | 高 | `minic.l`、`c99.l`、`--self-test` 均执行 |
...
5. 对回归/性能用例执行：
   - 运行 `resources/minic.l` / `resources/c99.l` 生成
   - 运行长输入 tokenization
   - 运行 `--self-test`
6. 把实际结果写入 `tests/lex/results/20260414-183656/`
...
### 5.11 `11_regression_c99_generate`

- 名称：`c99.l` 生成回归
- 目的：验证较大规格输入仍能稳定生成。
- 输入：
  - 外部输入：`resources/c99.l`
- 预期输出：
  - 生成成功
  - dot 文件齐全
  - 生成时间在阈值内
...
- 实际结果：
  - 通过
  - `elapsed_seconds=32`
- 结论分析：通过。`c99.l` 的生成时间明显高于 `minic.l`，但仍在本轮阈值内。

### 5.12 `12_runtime_perf_long_stream`
...
- 最长匹配、规则优先级、最小化语义保持、`minic/c99` 生成回归都已被外部测试覆盖
...
4. 可以把 `resources/minic.l` 和 `resources/c99.l` 的生成日志进一步提炼成结构化摘要，便于 CI 场景消费。

## 9. 产物清单

- 用例目录：`tests/lex/test_cases/`
- 预期目录：`tests/lex/expected/`
- 结果目录：`tests/lex/results/20260414-183656/`
- 执行脚本：`tests/lex/run_lex_tests.sh`
